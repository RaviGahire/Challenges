...
form.addEventListener("submit", function (e) {
  ...
  form.querySelectorAll(".form__error").forEach((err) => (err.textContent = ""));

  form.querySelectorAll("[required]").forEach((input) => {
    const group = input.closest(".form__group, .form__group--inline, .form__group--checkbox");

    const error = group.querySelector(".form__error");

    if (!input.checkValidity()) {
      error.textContent =
        input.type === "email"? "Please enter a valid email address, like example@domain.com": input.type === "checkbox"? "To submit this form, please consent to being contacted": "This field is required";
  ...
  const queryTypeError = form.querySelector(".form__group--inline .form__error");
